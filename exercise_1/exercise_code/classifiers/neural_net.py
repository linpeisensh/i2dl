...
#         layer1[layer1<=0] = 0
        layer1 = np.max(np.dot(X,W1) + b1, 0)
        ...
        p = exp_scores/ np.sum(exp_scores,axis = 1, keepdims = 1)
        loss = 0
        ...
        df[range(N),y] -= 1
        df /= len(y)
        grads['W2'] = np.dot(layer1.T,df)
        grads['W2'] +=  reg * W2
        grads['b2'] = np.sum(df,axis = 0)
        
        dlayer1 = np.dot(df, W2.T)
        dlayer1[layer1 <= 0] = 0
    ...
    def predict(self, X):
        ...
        ########################################################################
        hlayer = np.maximum(0,np.dot(X,self.params['W1'])+ self.params['b1'])
        f = np.dot(hlayer, self.params['W2']) + self.params['b2']
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    hidden_layersize = 100
    best_reg = 0.075
    step_reg = best_reg
    
    
    best = TwoLayerNet(input_size, hidden_layersize, num_classes)
    stats = best.train(X_train, y_train, X_val, y_val,
                                  num_iters=ti, batch_size=200,
                                  learning_rate=1e-3, learning_rate_decay=0.95,
                                  reg=0.108, verbose=True)
...
#         step_lr *= step_decay
#         step_reg *= step_decay
#         learning_rates = [best_lr+step_lr, best_lr, best_lr-step_lr]
#         regularization_strengths = [best_reg+step_reg, best_reg, best_reg-step_reg]
#         print(learning_rates)
#         print(regularization_strengths)
#         for lr in learning_rates:
#             for reg in regularization_strengths:
#     #             for hl in hidden_layersize:
#                 net = TwoLayerNet(input_size, hidden_layersize, num_classes)
...
#             bt_lr = best_lr
#             bt_reg = best_reg
#             print("best_val lr is %.4f, best_val reg is %.4f, best val accurary is %.3f" % (bt_lr, bt_reg, best_vg))
#         else:
#             print("best_val lr is %.4f, best_val reg is %.4f, best val accurary is %.3f" % (bt_lr, bt_reg, best_vg))
#             break
